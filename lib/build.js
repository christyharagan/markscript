...
function createTask(buildModel, buildConfig, server) {
    var configClient = server.getClient(buildConfig.database.configPort || 8002);
...
function deployTask(buildModel, buildConfig, server) {
    var adminClient = server.getClient(buildConfig.database.adminPort || 8001);
    var configClient = server.getClient(buildConfig.database.configPort || 8002);
...
function undeployTask(buildModel, buildConfig, server) {
    var client = server.getClient(buildConfig.database.httpPort || 8000);
...
function generateBuildModel(buildConfig, plugins, typeModel) {
    var buildModel = {
        databases: {},
        servers: {},
        ruleSets: [],
